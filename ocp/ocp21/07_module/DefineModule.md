...
By default, everything in a module is private to that module, even if the classes are public.

* exports: Allows other modules to use the public types in a specific package at compile-time and runtime.
* opens: Grants reflection-only access to a package. This is used for frameworks (like Spring, Hibernate, or Jackson) that need to look at private fields but don't need to link against the code normally.
```java
module com.myapp.display {
...
You must explicitly state which other modules your code needs.
...
* requires static: A "compile-time only" dependency (optional at runtime).
```java
...
Modules allow for a Service Provider Interface (SPI) pattern, which decouples the interface from the implementation.
...
* uses: The consuming module declares it will look for implementations of that service.
...
```
At runtime, the consumer uses ServiceLoader.load(MessageService.class) to find all available implementations.
## 5. Packing and Deploying
Once modularized, you can use specialized tools for deployment:
