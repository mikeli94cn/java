Serialization is the process of converting an object's state into a byte stream, while Deserialization is the process of reconstructing the object from that byte stream. 

This is primarily used for saving objects to files or sending them over a network.
## 1. Requirements for Serialization
For an object to be serialized, its class must implement the java.io.Serializable interface.
...
* Transient Keyword: If a field is marked as transient, it will not be serialized (useful for sensitive data like passwords or temporary cache).

## 2. Core Classes

* ObjectOutputStream: Contains the writeObject(Object obj) method to serialize an object to an underlying stream (like a file).
* ObjectInputStream: Contains the readObject() method to read the byte stream and recreate the object.
...
* Security: Native Java serialization is often criticized for security vulnerabilities. In modern enterprise applications, JSON or XML (using libraries like Jackson or Gson) is often preferred for data exchange.
