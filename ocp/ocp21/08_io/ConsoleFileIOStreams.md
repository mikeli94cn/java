In Java, the I/O API (java.io) uses the concept of Streams—a sequential flow of data from a source (input) to a destination (output). 

To handle different data types, Java provides two main stream hierarchies: Byte Streams (8-bit) for binary data and Character Streams (16-bit) for text. 
## 1. Console I/O
The System class provides three predefined stream variables for interacting with the console: 
...
## 2. File I/O
File operations are handled by classes that connect directly to a file system.

| Data Type| Input (Reading) | Output (Writing) |
|---|---|---|
